...
###############################################################################
    ...
    def horse_fastest_race(self, horse):
        ...
        if horse in self.race_dict:
            ...
            # Look for fastest race in its inner dictionary.
            fastest_race = None
            fastest_time = None
            for race, time in self.race_dict[horse].items():
                if fastest_race is None or time < fastest_time:
                    fastest_race = race
                    fastest_time = time
            
            # Update the fastest tuple.
            fastest = (fastest_race, fastest_time)
...
###############################################################################
    ...
    def get_average_time(self):
        ...
        '''
        # Loop through each horse in race_dict and calculate the average.
        averages = {}
        for horse, time_dict in self.race_dict.items():
            sum = 0
            for time in time_dict.values():
                sum += time
            averages[horse] = sum / len(time_dict)
        return averages
